...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (receivedData.equals("oil detected")) {  //and check level sensor
      oilDetected = true;
...
void pump() {
  // Check for oil
  if (oilDetected) {
    // If oil is detected, turn off the pump
    digitalWrite(pump1, LOW);
    Serial.println("Oil detected, pump turned off");
  } else {
    // If no oil is detected, turn on the pump
    digitalWrite(pump1, HIGH);
    Serial.println("No oil detected, pump turned on");
